...
def particle_initialization(start_state, width, num_particles, state_dim=2,init_with_true_state=False):
    ...
    if init_with_true_state:
        ...
        init_weights_log=compute_normal_density(initial_noise, 1.0)

        # log_c = - 0.5 * torch.log(torch.tensor(2 * np.pi)) - torch.log(torch.tensor(pos_noise))
        # log_mu_s = state_dim * log_c - 0.5 * (
        #     torch.sum(initial_noise ** 2 / (pos_noise) ** 2, dim=-1))  # shape: (batch_size, num_particle)
    else:
        bound_max = width / 2.0
        bound_min = -width / 2.0
